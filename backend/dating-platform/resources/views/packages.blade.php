            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <div class="generic_content active clearfix">
                        ...
                        <!--BUTTON START-->
                                                            <div class="generic_price_btn clearfix">
                                                                <a @if($discount_value === 100) href="/payments/free?pack_id={{$pack->id}}"
                                                                   @else href="/payments?pack_id={{$pack->id}}"
                                                                   @endif class="pack"
                                                                   ...
                                                                   onclick="FB_Checkout();">{{l("Purchase")}}</a>
                                                            </div>
                                                            <!--//BUTTON END-->
                        ...
                        <!--BUTTON START-->
                                                            <div class="generic_price_btn clearfix">
                                                                <a @if($discount_value === 100) href="/payments/free?pack_id={{$pack->id}}"
                                                                   ...
                                                                   data-description="{{$pack->name}}"
                                                                   data-currency="{{$pack->currency}}"
                                                                   onclick="FB_Checkout();">@if(Auth::user()->package() && Auth::user()->package()->id == $pack->id) {{l("Extend")}} @else {{l("Purchase")}} @endif</a>
                                                            </div>
                                                            <!--//BUTTON END-->
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div class="generic_content @if($pack->featured == 1) active @endif clearfix pk1_<?php echo $ii;?>">
                        ...
                        <!--BUTTON START-->
                                                    <div class="generic_price_btn clearfix">
                                                        @if(Auth::user()->package() && Auth::user()->package()->id == $pack->id && \App\Order::where(['package_id'=>Auth::user()->package()->id, 'user_id'=>Auth::user()->id, 'status'=>'Accepted'])->orderBy('id', 'desc')->whereNotNull('subscription_id')->limit(1)->count())
                                                            <a href="/payments/unsubscribe?pack_id={{$pack->id}}"
                                                               onclick="return confirm('{{l("Are you sure you want to unsubscribe?")}}');FB_Checkout();">{{l("Unsubscribe")}}</a>
                                                        @else
                                                            <a @if($discount_value === 100) href="/payments/free?pack_id={{$pack->id}}"
                                                               @else href="/payments?pack_id={{$pack->id}}"
                                                               @endif class="pack" data-discount="{{$discount_value}}"
                                                               data-pack="{{$pack->id}}" data-price="{{$pack->price}}"
                                                               data-description="{{$pack->name}}"
                                                               data-currency="{{$pack->currency}}"
                                                               data-priceid="{{$pack->stripe_price_id}}"
                                                               onclick="FB_Checkout();">@if(Auth::user()->package() && Auth::user()->package()->id == $pack->id) {{l("Extend")}} @else {{l("Purchase")}} @endif </a>
                                                        @endif
                                                    </div>
                                                    <!--//BUTTON END-->
                    ...
                    <div class="generic_content @if($pack->featured == 1) active @endif clearfix pk2_<?php echo $iii;?>">
                        ...
                        <div class="generic_head_price clearfix">
                            ...
                            <div class="generic_price_tag clearfix">    
                                ...
                                @php
                                	$pprice = ($pack->type == 'credits') ? $pack->new_price/$pack->credits : $pack->new_price/$pack->duration;
                                @endphp
                            @else   
                                <span class="price">
                                    <span class="sign">@switch($pack->currency) @case('EUR') € @break @case('USD') $ @break @default @endswitch</span>
                                ...
                                @php
                                    if($pack->front_price != null && $pack->front_price!=0) 
                                    {
                                        $pprice = $pack->front_price; 
                                    }
                                    else 
                                    {
                                        if($pack->type == 'credits' || $pack->type == 'subscription-credits') 
                                        {
                                            $pprice = $pack->price/$pack->credits; 
                                        }
                                        else
                                        { 
                                            $pprice = $pack->price/$pack->duration;
                                        } 
                                    }
                                @endphp
                            @endif
                        ...
                        <div class="generic_feature_list">
                            <ul>
                                ...
                                <li><span class="sign">@switch($pack->currency) @case('EUR') € @break @case('USD') $ @break @default @endswitch</span> {{ number_format($pprice * $pack->credits, 0, '.', ',') }}</li>
                            </ul>
                        </div>
                        <!--//FEATURE LIST END-->
                        ...
                        <!--BUTTON START-->
                                                    <div class="generic_price_btn clearfix">
                                                        <a @if($discount_value === 100) href="/payments/free?pack_id={{$pack->id}}"
                                                           ...
                                                           data-description="{{$pack->name}}"
                                                           data-currency="{{$pack->currency}}"
                                                           {!!$pack->type == 'subscription-credits' ? 'data-priceid="'.$pack->stripe_price_id.'"' : ''!!} onclick="FB_Checkout();">{{l("Purchase")}}</a>
                                                    </div>
                                                    <!--//BUTTON END-->
...
.divs {
    ...
    background: #6D6D6D;
    color:#ffffff;
    font-size: 20px; padding:3px; text-align:center;
    height: 40px;
}
.divs:hover, #selected_divs, .QuickStartLong:hover {
    background: #898989;
}
</style>
<div class="modal fade" id="payment-selector" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
    <div class="modal-dialog window-popup create-photo-album" role="document">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h6 class="title">{{l("Select Payment method")}}</h6>
            </div>

            <div class="modal-body payment-selector-div">
                @foreach($active_payments as $k => $active_payment)
                 <div class="divs" data-payment_method="{{$k}}">
                 @if($active_payment=="Wire-transfer")<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M2 20h20v2H2v-2zm2-8h2v7H4v-7zm5 0h2v7H9v-7zm4 0h2v7h-2v-7zm5 0h2v7h-2v-7zM2 7l10-5 10 5v4H2V7zm2 1.236V9h16v-.764l-8-4-8 4zM12 8a1 1 0 1 1 0-2 1 1 0 0 1 0 2z" fill="currentColor"></path></svg>
                 @elseif($active_payment=="Stripe")<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M3 3h18a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1zm17 9H4v7h16v-7zm0-4V5H4v3h16z" fill="currentColor"></path></svg>
                 @elseif($active_payment=="Paypal")<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M20.067 8.478c.492.88.556 2.014.3 3.327-.74 3.806-3.276 5.12-6.514 5.12h-.5a.805.805 0 0 0-.794.68l-.04.22-.63 3.993-.032.17a.804.804 0 0 1-.794.679H7.72a.483.483 0 0 1-.477-.558L7.418 21h1.518l.95-6.02h1.385c4.678 0 7.75-2.203 8.796-6.502zm-2.96-5.09c.762.868.983 1.81.752 3.285-.019.123-.04.24-.062.36-.735 3.773-3.089 5.446-6.956 5.446H8.957c-.63 0-1.174.414-1.354 1.002l-.014-.002-.93 5.894H3.121a.051.051 0 0 1-.05-.06l2.598-16.51A.95.95 0 0 1 6.607 2h5.976c2.183 0 3.716.469 4.523 1.388z" fill="currentColor"></path></svg>
                 @else<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M3 3h18a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1zm17 9H4v7h16v-7zm0-4V5H4v3h16z" fill="currentColor"></path></svg>
                 @endif
                 &nbsp;{{$active_payment}}</div>
                {{-- <input type="radio" name="payment_method" id="payment_method_{{$k}}" value="{{$k}}" id="" /> <label for="{{$k}}">{{$active_payment}}</label> --}}
                @endforeach
                <input type="submit" class="btn btn-primary btn-lg" id="btn_select_payment_method" value="{{l("Purchase")}}">
            </div>
        </div>
    </div>
</div>
...
@section('scripts')
    ...
    $('.payment-selector-div').on('click','.divs',function () {
        $(this).parent().find('.divs').css('background-color', '');
        $(this).css('background-color', '#CA0404');
    });
