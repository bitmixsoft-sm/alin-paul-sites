    var callingObj = {
        ...
        disableVideo: function () {
            $('.popup-chat[data-id="'+callingObj.inCallWith+'"] .video-controls .remove_video .fas').removeClass('fa-video').addClass('fa-video-slash');
        },
        disableAudio: function () {
            $('.popup-chat[data-id="'+callingObj.inCallWith+'"] .video-controls .mute_mic .fas').removeClass('fa-microphone').addClass('fa-microphone-slash');
        }
    };
    var in_call = false;
  	var inp = false;
    ...
    function send_call(t){
		...
		send_call_from = $(t).attr('data-from');
        ...
        extraQuery = '';
        if (send_call_from != undefined && send_call_from !='') {
            extraQuery = '&call_from='+send_call_from;
            callingObj.callFrom = send_call_from;
            callingObj.caller = send_call_from;
        }
		$('.popup-chat[data-id="'+inp+'"] .ubgvideo').html('<iframe src="/videochat?call_to='+inp+extraQuery+'&source=iframe#init" frameborder="0" width="100%" height="100%"></iframe>');
	...
	channel.bind('incomingCall', function(data) {
		...
		if(!in_call){
            ...
            callingObj.outputCaller = data_caller.ready;
            if (callingObj.myId == '') {
	...
	$('#answer_call').click(function(){
        ...
        if (data_caller.callFrom != undefined && data_caller.callFrom != '') {
            callingObj.callFrom = data_caller.callFrom;
        }
	...
	$('#refuse_call').click(function(){
		$('#is_calling').modal('hide');
            console.log('Refuse call', data_caller);
		...
		var CSRF_TOKEN = $('input[name="_token"]').val();
					$.ajax({
			                    ...
			                    data: {_token: CSRF_TOKEN,
			                    		to: (callingObj.callFrom!='')?callingObj.callFrom:data_caller.from
			                    	},
			                    dataType: 'JSON',
	...
	function close_video_call(uid, home)
	{
        ...
        console.log('Closing video', callingObj);
        ...
        if (in_call == true) {
            ...
            if (callingObj.inCallWith!='') {
                ...
                if ($('.popup-chat[data-'+propNameCheck+'="'+callingObj.inCallWith+'"] .ubgvideo').html()!='') {
                    ...
                    $('.popup-chat[data-'+propNameCheck+'="'+callingObj.inCallWith+'"] .videocallicon').html('<i class="fas fa-phone" onclick="send_call(this)" data-call="'+callingObj.inCallWith+'""'+((callingObj.callFrom != '')?' data-from="'+callingObj.callFrom+'"':'')+'></i>');
