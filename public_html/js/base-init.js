...
CRUMINA.Bootstrap = function () {
	...
	if (date_select_field.length) {
		var adultLimit = moment().subtract(18, 'years');
        date_select_field.daterangepicker({
            startDate: adultLimit,
            maxDate: adultLimit,
...
$(document).ready(function () {
	...
	var userSearchInput = $('input[tabindex]').val();
	var CSRF_TOKEN = $('input[name="_token"]').val();
	$('input[tabindex]').keyup(function(){
			...
			render: {
				option: function(item, escape) {
					return '<div class="inline-items">' +
						...
						'</div>'+
						(item.icon ? '<span class="notification-icon"><svg class="' + escape(item.icon) + '"><use xlink:href="/svg-icons/sprites/icons.svg#' + escape(item.icon) + '"></use></svg></span>' : '') +
						'</div>';
				},
				item: function(item, escape) {
					var label = item.name;
					return window.location.href = label;
				}
			}
		});
	}
		...
		swipers['swiper-' + index] = new Swiper('.swiper-' + index, {
			...
			onSlideChangeStart: function (swiper) {
				...
				if (sliderThumbs.length) {
					...
					update_slider_desc(sliderThumbs.find('.slides-item').eq(realIndex).attr('data-desc'));
...
CRUMINA.mediaPopups = function () {
	...
	$('.photo-zoom-gallery').each(function () {
		$(this).magnificPopup({
			delegate: '.photo-item',
			type: 'image',
			gallery: {
...
$(document).ready(function () {
    $('input[name="phone"]').closest('.form-group').hide();
});